#!/usr/bin/env python3
# ============================================================
#  demos/03_astar_manhattan.py  —  A* with Manhattan heuristic
# ============================================================
#  h(n) = |Δrow| + |Δcol|
#  Admissible: never overestimates (no diagonal moves allowed).
#  Tends to explore fewer cells than BFS on most mazes.
# ============================================================
...
def main():
    parser = argparse.ArgumentParser(description='A* (Manhattan) maze solver')
    ...
    solver = AStarSolver(m, heuristic='manhattan',
    ...
    if args.save:
        csv_path = save_stats(
            algorithm='A*(Manhattan)', maze_size=size,
    ...
    a = agent(m, footprints=True, shape='arrow', color=COLOR.green, filled=True)
    if wm:
        wm.draw()
        wm.keep_text_on_top()

    textLabel(m, 'Algorithm',      'A* (Manhattan)')
    textLabel(m, 'Mode',           'weighted' if weighted else 'unweighted')
    textLabel(m, 'Heuristic',      'h = |Δrow| + |Δcol|')
