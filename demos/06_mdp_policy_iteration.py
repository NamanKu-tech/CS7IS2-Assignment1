#!/usr/bin/env python3
# ============================================================
#  demos/06_mdp_policy_iteration.py  —  MDP Policy Iteration
# ============================================================
#  Alternates between:
#    Policy Evaluation  — compute V for current fixed policy
#    Policy Improvement — greedily update policy from V
#  Stops when policy doesn't change.
#
#  Fewer outer iterations than Value Iteration, but each
#  evaluation step is more expensive.
# ============================================================
import sys, os, time, argparse
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))

from pyamaze import agent, COLOR, textLabel
from solvers import MDPPolicyIterationSolver
...
def main():
    parser = argparse.ArgumentParser(description='MDP Policy Iteration maze solver')
    ...
    solver = MDPPolicyIterationSolver(
        m, gamma=GAMMA, noise=NOISE, living_reward=LIVING_REWARD
    )
    ...
    print(f"Converged in  : {stats['iterations']} policy iterations")
    ...
    if args.save:
        csv_path = save_stats(
            algorithm='MDP-PI', maze_size=size,
    ...
    textLabel(m, 'Algorithm',   'MDP Policy Iteration')
