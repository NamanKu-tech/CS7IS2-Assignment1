...
def main():
    parser = argparse.ArgumentParser(description='BFS maze solver')
    ...
    a = agent(m, footprints=True, shape='arrow', color=COLOR.blue, filled=True)
    if wm:
        wm.draw()
        wm.keep_text_on_top()

    textLabel(m, 'Algorithm',      'BFS')
